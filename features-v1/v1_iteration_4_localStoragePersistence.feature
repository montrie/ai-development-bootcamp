Feature: localStorage Persistence
  ...
  I want my tasks to be saved between page refreshes
  ...
  Scenario: The app continues to work in-memory when localStorage is unavailable
    Given localStorage is unavailable
    When I enter "Buy milk" into the todo input
    And I click the add button
    Then I should see a todo item with text "Buy milk"

  Scenario: A storage notice is shown when localStorage is unavailable
    Given localStorage is unavailable
    Then I should see a storage unavailable notice
